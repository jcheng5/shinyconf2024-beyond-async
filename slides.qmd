...
* Use {future} (by Hernik Bengtsson) to perform long-running operations in a separate R process that runs in the background.
* Use {promises} to handle the results of these operations, back in the original R process, in a principled way.
* Deeply integrate support for promises into Shiny's reactive graph.
...
* Async operations "infect" everything downstream of them; any code that calls an async function must become an async function. See ["What color is your function?"](https://journal.stuffwithstuff.com/2015/02/01/what-color-is-your-function/) by Bob Nystrom.
...
[**{mirai}**](https://shikokuchuo.net/mirai/) is a new package by Charlie Guo that, like {future}, can run code in a background R process.
...
The author of {future} informs me that a {future.mirai} package is currently in beta, and combines the convenient API of {future} with the low overhead of {mirai}.
